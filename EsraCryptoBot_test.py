...
async def execute_all_tri_arb_orders1(list_of_lists_of_arb_lists):
    #Determines profitability and executes profitable orders.
    profit_spread_list = []
    profit_dollars_list = []
    quantity_list1 =[]
...
async def find_tri_arb_opp1(exchange, total_markets, arb_list, fee_percentage = .001):
    ...
    if arb_list[2] == arb_list[0].split('/')[1] + '/' + arb_list[1].split('/')[1]:
        max_bid = await maxBid(exchange, arb_list[0], total_markets, count)
        min_ask = await minAsk(exchange, arb_list[1], total_markets, count)
        max_bid1 = await maxBid(exchange, arb_list[2], total_markets, count)
        if min_ask['isFound'] and max_bid['isFound'] and max_bid1['isFound']:
            spread = ((1/max_bid['bid_price']) * min_ask['ask_price'] * (1/max_bid1['bid_price'])) - 1
            print(spread)
            ...
            if spread > 0:
                if fee_adjusted_spread > 0:
                    ...
                    print("QUANTITY OF STARTING COIN (BEST CASE): {} \n".format(max_bid['bid_price'] * max_bid['bid_quantity']))
                    arbitrage = {
                        'exchange': exchange,
                        'exch_rate_list': [max_bid['bid_price'], min_ask['ask_price'], max_bid1['bid_price']],
                        'sym_list': arb_list,
                        'profit': spread,
                        'quantity_list': [max_bid['bid_quantity'], min_ask['ask_quantity'], max_bid1['bid_quantity']],
        ...
        if min_ask['isFound'] and max_bid['isFound'] and min_ask1['isFound']:
            spread = ((1/max_bid['bid_price']) * min_ask['ask_price'] * min_ask1['ask_price']) - 1
            print(spread)
            fee_adjusted_spread = spread - (fee_percentage*5)
            if spread > 0:
                if fee_adjusted_spread > 0:
                    ...
                    arbitrage = {
                        'exchange': exchange,
                        'exch_rate_list': [max_bid['bid_price'], min_ask['ask_price'], min_ask1['ask_price']],
                        'sym_list': arb_list,
                        'profit': spread,
                        'quantity_list': [max_bid['bid_quantity'], min_ask['ask_quantity'], min_ask1['ask_quantity']],
                        'fee_percentage': fee_percentage
                    }
                    return arbitrage
...
async def pre_tri_arb_USD_transfer(exchange, sym_list, fee_percentage, initial_quantity): #make exchange, exch_rate_list, sym_list, fee_percentage global vars
    ...
    try:
        ...
        checkforOpenOrders(exchange, USDmarket)
...
async def pre_tri_arb_USD_transfer1(exchange, sym_list, fee_percentage, dollar_exch_bid, dollar_exch_quantity): #make exchange, exch_rate_list, sym_list, fee_percentage global vars
    market1 = sym_list[0]
    USDmarket = market1[0:3] + "/USDC"
    print("Transferring capital in USDC to " + market1)
    try:
        ...
        if exchange.has['createMarketOrder']:
            pre_USD_transfer_order = exchange.create_order(symbol=USDmarket,
                        side=SIDE_SELL,
                        type=ORDER_TYPE_LIMIT,
                        quantity=dollar_exch_quantity, #compensating for fees so we receive the correct quantity_1
                        price=dollar_exch_bid,
                        timeInForce=TIME_IN_FORCE_GTC)
        else:
            print("Could not complete order.")
        print("Pre Tri Arb Coin Transfer Complete")
...
def tri_arb_orders(exchange, exch_rate_list, sym_list, quantity_list, fee_percentage): #exch_rate_list is the exchange rates for the tri arb, sym_list are the 3 markets in the tri arb
    ...
    checkforOpenOrders(exchange, quantity_list[0])
    if exchange.has['createMarketOrder']:
    ...
    checkforOpenOrders(exchange, quantity_list[1])
    if exchange.has['createMarketOrder']:
    ...
    checkforOpenOrders(exchange, quantity_list[2])
    if exchange.has['createMarketOrder']:
...
async def checkforOpenOrders(exchange, market):
    ...
    isOrders = True
    while isOrders:
        if exchange.fetchOpenOrders(symbol = market) == "":
            print("Order is Complete")
            return True
        time.sleep(1)
        i++
        if i > secondsPassedinOrder - 1:
            return False
